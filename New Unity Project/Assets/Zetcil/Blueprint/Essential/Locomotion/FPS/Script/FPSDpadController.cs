...
namespace Zetcil
{
    public class FPSDpadController : MonoBehaviour, IDragHandler, IPointerUpHandler, IPointerDownHandler
    {
        ...
        public enum DpadModeEnum { Fixed, Dynamic, Following };
        [Tooltip("Fixed: It doesn't move. " +
            "\nDynamic: Every time the Dpad area is pressed, the Dpad position is set on the touched position. " +
            "\nFollowing: If the finger moves outside the Dpad background, the Dpad follows it.")]

        [Header("Dpad Settings")]
        public DpadModeEnum DpadMode = DpadModeEnum.Fixed;
        ...
        [Header("Directional Dpad")]

        [Tooltip("Number of directions of the Dpad. \nKeep at 0 for a free Dpad.")]
        [Range(0, 12)]
        ...
        {
            if (DpadMode != DpadModeEnum.Fixed)
            {
        ...
        public void OnBackgroundDrag(BaseEventData baseEventData)
        {
            ...
            if (DpadMode == DpadModeEnum.Following)
                Following(vector);
